def build_sandbox(lua):
    sandbox = lua.eval('{}')
    lua.globals().sandbox = sandbox
    sandbox.allowed_require_names = lua.eval('{}')
    ...
    sandbox.env = """
    {
      ...
      require = function(name)
        if sandbox.allowed_require_names[name] then
          ok, res = pcall(function() return require(name) end)
          if ok then
            return res
          end
        end
    ...
    sandbox.fix_metatables = lua.eval("""
    function()
    ...
    sandbox.enable_memory_limit = lua.eval("""
    function ()
    ...
    sandbox.enable_per_instruction_limit = """
    function ()
      function _debug_step(event, line)
    ...
    sandbox.create_coroutine = lua.eval("""
    function (f, ...)
    ...
    sandbox.run = lua.eval("""
    function (untrusted_code)
      sandbox.fix_metatables()
      sandbox.enable_memory_limit()
      sandbox.enable_per_instruction_limits()
      untrusted_function, message = load(untrusted_code, nil, 't', sandbox.env)
